...
- **Karte** (Leaflet/OpenStreetMap, kostenlos) – wird nur geladen, wenn der
  Karte-Bereich tatsächlich geöffnet wird
...
- Aufgaben (mit Haken, den JEDES Familienmitglied setzen kann, unabhängig
  davon wer die Aufgabe angelegt hat), optional **wiederkehrend**
  (täglich/wöchentlich/monatlich) – beim Abhaken entsteht automatisch die
  nächste offene Ausgabe; **Fairness-Ansicht** zeigt, wer wie viel erledigt hat
- Chat mit 💛-Reaktionen, **Fotos** (werden vor dem Speichern automatisch
  verkleinert/komprimiert) und **Sprachnachrichten** (Mikrofon-Aufnahme,
  max. 60s) – alles anlegen, bearbeiten, löschen, automatisch neueste
  zuerst; ab dem 11. Eintrag klappen sich ältere in einen Stapel ein
- Gemeinsamer Kalender mit echter Monatsansicht: jedes Familienmitglied kann
  Termine (Titel, Datum, Uhrzeit) eintragen, bearbeiten und löschen; Termine
  können **mehrtägig** sein (Start-/Enddatum) und **wiederkehrend**
  (wöchentlich/monatlich/jährlich – z. B. für Geburtstage/Jahrestage); Tage
  mit Terminen sind im Monatsraster mit einem Punkt markiert, antippen zeigt
  die Termine des Tages; der nächste anstehende Termin erscheint zusätzlich
  als Karte auf der Startseite; **.ics-Export** für externe Kalender-Apps
- **Wochenrückblick** auf der Startseite (erledigte Aufgaben, neue Termine,
...
- Online-Status pro Mitglied (grüner Punkt am Namenskürzel) + einmalige Anmeldung
  pro Gerät ("wer bist du"), Abmelden/Wieder-anmelden über Familien-Code
- Namen frei einstellbar (oben im Banner antippen); Mitglieder werden nur
  durch ihre Namensinitiale dargestellt, keine Avatar-Symbole; beim
  allerersten Start verpflichtend, bevor die App nutzbar ist
...
- **Kurzes Onboarding** beim allerersten Öffnen, danach das verpflichtende
  Namen/"wer bist du"-Setup (danach nie wieder)
